...
class GradientGlowFilter extends BitmapFilter
{
	...
	/**
		An array of colors that defines a gradient. For example, red is `0xFF0000`, blue is
		`0x0000FF`, and so on.
	...
	/**
		...
		@return `BitmapFilter` — A new `GradientGlowFilter` instance with all the same properties as the original `GradientGlowFilter` instance.
	 */
	...
	@:noCompletion private override function __initShader(renderer:DisplayObjectRenderer, pass:Int, sourceBitmapData:BitmapData):Shader
	{
		...
		if (pass == numBlurPasses)
		{
			...
			__colorFadeBmp.setPixels(__colorFadeBmp.rect, __colorFadeArr);
			shader.colorGradient.input = __colorFadeBmp;
...
@:noDebug
#end
private class ColorRatioShader extends BitmapFilterShader
{
	@:glFragmentSource("
...
@:noDebug
#end
private class FullCombineShader extends BitmapFilterShader
{
	@:glFragmentSource("
		uniform sampler2D openfl_Texture;
