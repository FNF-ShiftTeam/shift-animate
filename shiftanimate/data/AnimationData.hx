package shiftanimate.data;

import shiftanimate.effects.*;
import shiftanimate.motion.AdjustColor;
			...
			case "DSF", "DropShadowFilter":
				{
					...
					return new openfl.filters.DropShadowFilter(drop.DST, drop.AL, colorFromString(drop.C), drop.A, drop.BLX, drop.BLY, drop.STR, drop.Q,
						drop.IN, drop.KK);
				}
			case "GF", "GlowFilter":
				{
			...
			case "BF", "BevelFilter": // Friday Night Funkin reference ?!??!?!''1'!'?1'1''?1''
				{
					...
					return new shiftanimate.filters.BevelFilter(bevel.DST, bevel.AL, colorFromString(bevel.HC), bevel.HA, colorFromString(bevel.SC), bevel.SA,
						bevel.BLX, bevel.BLY, bevel.STR, bevel.Q, bevel.TP, bevel.KK);
				}
			case "BLF", "BlurFilter":
				{
			...
			case "GGF", "GradientGlowFilter":
				{
					...
					return new shiftanimate.filters.GradientGlowFilter(gradient.DST, gradient.AL, colors, alphas, ratios, gradient.BLX, gradient.BLY,
						gradient.STR, gradient.Q, gradient.TP, gradient.KK);
				}
			case "GBF", "GradientBevelFilter":
				{
					...
					return new shiftanimate.filters.GradientBevelFilter(gradient.DST, gradient.AL, colors, alphas, ratios, gradient.BLX, gradient.BLY,
						gradient.STR, gradient.Q, gradient.TP, gradient.KK);
				}
		}
...
abstract Filters({})
{
	 ...
	 * @see shiftanimate.motion.AdjustColor
	 * @see shiftanimate.motion.ColorMatrix
	 * @see shiftanimate.motion.DynamicMatrix
